//
//  BookingHotelPendingViewC.swift
//  Rush Rooms
//
...
class BookingHotelPendingViewC: UIViewController {
    ...
    @IBOutlet weak var cancelBookingBtn, backBtn, favBtn, writeReviewBtn, chatHostBtn, supportBtn, checkInBtn, confirmedBtn: UIButton!
    ...
    private func setUpView(){
        ...
        GlobalMethods.shared.provideCornerRadius(view: self.confirmedBtn, cornerRadius: FontSize.size_4.rawValue, borderColor: UIColor.colorWith(hexString: .CLEAR).cgColor, borderWidth: ZERO)
    }
...
//    }
    ...
    @IBAction func checkInBtn_Action(_ sender: Any) {
        let viewC = DIConfigurator.sharedInst().getCheckInViewC()
        viewC.modalTransitionStyle = .crossDissolve
        viewC.modalPresentationStyle = .overCurrentContext
        self.navigationController?.present(viewC, animated: true)
    }
    
    @IBAction func confirmedBtn_Action(_ sender: Any) {
